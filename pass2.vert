...

{
  ...
  gl_Position = OCS_to_CCS * vec4(vertPosition, 1.0f);   // CHANGE THIS
  ...
  normal = vec3(OCS_to_WCS * vec4(vertNormal, 0.0f));		// CHANGE THIS

  // Calculate position in WCS

  wcsPosition = vec3(OCS_to_WCS * vec4(vertPosition, 1.0f));	// CHANGE THIS
